...
namespace yDVR{
  const Vector& SincDVR::grids(){
    if(grids_.size() == 0){
      // ::indent();
      // << "Calculate Sinc-DVR grids..." << std::endl;
  ...
  const Matrix& SincDVR::kineticMatrix(){
    if(kinetic_matrix_.cols() == 0){
      // ::indent();
      // << "Calculate Sinc-DVR kinetic matrix..." << std::endl;
      Scalar deltax=(b_-a_)/N_plus_1_; 
  ...
  {
    // << std::endl;
    // << "============================ class sincDVR ============================" << std::endl;
    // << std::endl;
    // << "Reference: "<< std::endl;
    // << "  D. T. Colbert and W. H. Miller, J. Chem. Phys. 96, 1982 (1992) doi:10.1063/1.462100" << std::endl;
    // << std::endl;
  }
}
